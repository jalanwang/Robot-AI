...
dbPath = os.path.join(BASE_DIR, 'mynamecard.db')
...
# class Person:
#     def __init__(self, id=None, name="", pnumber=""): # id=None에 유의
#         self.id = id
#         self.name = name
#         self.pnumber = pnumber
...
class PersonRepository: #클래스 이름
    ...
    def findById(self, id: int) -> Person | None: # 수정을 위해서 입력 받은 id가 존재하는 확인하는 함수
                                                # 객체반환타입을 명시했다. Person이거나 None
        conn = self.getConnection()
        cur = conn.cursor()
